		...
		if (this.files[0].size > maks) {
			...
			switch(kontrol){
				case "fotokondisi":
					...
					target_proses = serverloc + "/permintaan/uploadberkas/fotokondisi";
					break;
				case "buktibayar":
					...
					target_proses = serverloc + "/permintaan/uploadberkas/buktibayar";
					break;
				case "izinskhplama":
					...
					target_proses = serverloc + "/permintaan/uploadberkas/"+layanan;
					break;
				case "suratpermohonan":
					...
					target_proses = serverloc + "/permintaan/uploadberkas/suratpermohonan";
					break;
				case "cerapan":
					...
					target_proses = serverloc + "/permintaan/uploadberkas/cerapan";
					break;
			}
...
$("#pilih-tambahan").change(function () {
	let kontrol = $('#kontrol').val();
	let target_proses = serverloc + "/permintaan/uploadtambahan/"+kontrol;
	let maks = 2000000;
	let alert_maks = "Ukuran file melebihi 2 Mb!";
		...
		switch (kontrol) {
			case "fotokondisi":
				...
				if(pdflow == true || pdfup == true){
					$(".btn-lihat").attr("href", serverloc + "/permintaan/viewPdf/" + nama_file + "/" + kontrol);
				} else {
			...
			case "buktibayar":
				...
				if(pdflow == true || pdfup == true){
					$(".btn-lihat2").attr("href", serverloc + "/permintaan/viewPdf/" + nama_file + "/" + kontrol);
				} else {
			...
			case "izinskhplama":
				...
				if(pdflow == true || pdfup == true){
					$(".btn-lihat3").attr("href", serverloc + "/permintaan/viewPdf/" + nama_file + "/" + layanan);
				} else {
					switch(layanan){
						case "Tera":
			...
			case "suratpermohonan":
				...
				if(pdflow == true || pdfup == true){
					$(".btn-lihat4").attr("href", serverloc + "/permintaan/viewPdf/" + nama_file + "/" + kontrol);
				} else {
			...
			case "cerapan":
				...
				if(pdflow == true || pdfup == true){
					$(".btn-lihat5").attr("href", serverloc + "/permintaan/viewPdf/" + nama_file + "/" + kontrol);
				} else {
...
function completeHandler_tambahan(event) {
	...
	if (nama_file == "gagal") {
		...
		if(pdflow == true || pdfup == true){
			$(".btn-lihat_"+kontrol).attr("href", serverloc + "/permintaan/viewPdftambahan/" + nama_file);
		} else {
...
function batal_berkas(kontrol) {
    ...
    if(confirm('Hapus file upload ' + namakontrol + '?') == true){
        switch (kontrol) {
            case "fotokondisi":
                namafile = $("#berkas").val();
                url = serverloc + "/permintaan/batalberkas/" + namafile + "/fotokondisi";
            ...
            case "buktibayar":
                ...
                url = serverloc + "/permintaan/batalberkas/" + namafile + "/buktibayar";
			...
			case "izinskhplama":
				...
				switch(layanan){
					case "Tera":
						url = serverloc + "/permintaan/batalberkas/" + namafile + "/izinpersetujuantipe";
						break;
					case "Tera Ulang":
						url = serverloc + "/permintaan/batalberkas/" + namafile + "/skhplama";
						break;
				}
			...
			case "suratpermohonan":
				...
				url = serverloc + "/permintaan/batalberkas/" + namafile + "/suratpermohonan";
			...
			case "cerapan":
				...
				url = serverloc + "/permintaan/batalberkas/" + namafile + "/cerapan";
			...
			default:
				...
				url = serverloc + "/permintaan/batalberkas/" + namafile + "/doktambahanpengajuan";
